...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --project="$PROJECT_ID" \
  --region="$REGION" \
  --set-env-vars="GOOGLE_GENAI_USE_VERTEXAI=1,GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=${REGION},CLIENT_ID=${CLIENT_ID},CLIENT_SECRET=${CLIENT_SECRET},AUTH_URI=${AUTH_URI},SCOPES=${SCOPES},TOKEN_URI=${TOKEN_URI},WELL_KNOWN_URI=${WELL_KNOWN_URI},GOOGLE_CLOUD_AGENT_ENGINE_ENABLE_TELEMETRY=${GOOGLE_CLOUD_AGENT_ENGINE_ENABLE_TELEMETRY},OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=${OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT}" \
